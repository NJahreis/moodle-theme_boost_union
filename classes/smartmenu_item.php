...
class smartmenu_item {
    ...
    /**
     * Generate the item as doc link item. Use page_doc_link to get the link and generate node data.
     * Page_doc_link returns a html::tag string so extraction of the href is necessary
     * @return array
     */
    protected function generate_docs_item() {
        
        $docurlhtml = page_doc_link($this->item->title);
        preg_match_all('/(^.*href=")([^"]*)(.*)$/', $docurlhtml, $docurl);
        return $this->generate_node_data(
            $this->item->title,
            $docurl[2][0],
            null,
    ...
    public function build() {
        ...
        // Menu item class.
        $types = [self::TYPESTATIC => 'static', self::TYPEDYNAMIC => 'dynamic', self::TYPEHEADING => 'heading', self::TYPEDOCS => 'docs'];
